 ...
 * 
 * This script creates a fully automated loop:
 * 1. Run tests → 2. Capture errors → 3. AI analyzes → 4. Apply fixes → 5. Repeat
 * 
 * Uses DeepSeek API (you already have a key) for AI-powered fixes.
 * 
 * Usage:
 *   node scripts/auto-fix-loop.js
 *   node scripts/auto-fix-loop.js --test "browser-test.js"
 *   node scripts/auto-fix-loop.js --max-iterations 5
 *   node scripts/auto-fix-loop.js --dry-run  # Just analyze, don't apply fixes
 */
...
const DEFAULT_CONFIG = {
  ...
  timeoutPerTest: 120000,  // 2 minutes
};
...
async function generateFix(testOutput, iteration) {
  ...
  const systemPrompt = `You are an expert debugging assistant. Your job is to:
1. Analyze test failures and errors
2. Identify the root cause of each issue
3. Provide precise, minimal code fixes
4. Explain what was wrong and how your fix addresses it

IMPORTANT:
- Only fix issues mentioned in the test output
- Provide complete, working code snippets
- Follow existing code conventions
- Never break existing functionality
- If the issue is configuration-related, provide the exact config changes
  ...
  "fixes": [
    {
      ...
      "oldCode": "// Comment describing what to replace",
      "newCode": "The complete new code to replace it"
    }
  ],
  "confidence": "high|medium|low"
}`;

  const userPrompt = `Test Output (Iteration ${iteration}):
${'='.repeat(60)}
...
Please analyze the test failures above and provide fixes.
Focus on the most critical issues first.
Maximum ${DEFAULT_CONFIG.maxFixesPerIteration} fixes per iteration.
...
Return ONLY valid JSON, no markdown formatting.`;
  ...
  try {
    ...
    try {
      ...
      log(`Raw response: ${content.slice(0, 500)}...`, 'error');
  ...
  for (const fix of fixPlan.fixes) {
    ...
    try {
      ...
      if (fix.oldCode && fix.oldCode.trim()) {
        ...
        // Replace entire file content (risky, but sometimes necessary)
...
async function runAutoFixLoop() {
  console.log(`
╔══════════════════════════════════════════════════════════╗
║         🔄 Auto-Fix Loop - Automated Debugging           ║
╠══════════════════════════════════════════════════════════╣
...
╚══════════════════════════════════════════════════════════╝
  ...
  while (iteration < DEFAULT_CONFIG.maxIterations) {
    ...
    // Save test output to file for reference
    ...
    // Step 7: Wait before next iteration (avoid API rate limits)
...
Usage:
  node scripts/auto-fix-loop.js [options]
...
Examples:
  node scripts/auto-fix-loop.js
  node scripts/auto-fix-loop.js --test browser-test.js
  node scripts/auto-fix-loop.js --max-iterations 5 --dry-run
`);
      process.exit(0);
    }
  }
