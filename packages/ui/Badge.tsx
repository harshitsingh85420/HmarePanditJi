...
export const Badge = React.forwardRef<HTMLSpanElement, BadgeProps>(
  (
    ...
    const variantClasses = {
      ...
      primary:
        "bg-[var(--badge-primary-light)] text-[var(--badge-primary-dark)]",
    };
    ...
    return (
      <span
        ref={ref}
        style={styleVars}
        className={`inline-flex items-center rounded-full font-medium ${sizeClass} ${variantClasses[variant]} ${className}`}
        {...props}
      >
        {dot && (
          <span
            className={`${dotSizeClass} mr-1.5 rounded-full ${dotClasses[variant]}`}
            aria-hidden="true"
          />
        )}
        {children}
