...
export function PanchangStrip({ tithi = "" }: PanchangStripProps) {
  ...
  return (
    <div className="flex items-center justify-between bg-saffron-50 rounded-card px-4 py-2">
      <span className="t-hint text-softgrey font-hindi">
        🗓️ {p.hindiWeekday}, {p.hindiDate}
      </span>
      {p.tithi && <span className="t-hint text-softgrey font-hindi">{p.tithi}</span>}
    </div>
  );
}
