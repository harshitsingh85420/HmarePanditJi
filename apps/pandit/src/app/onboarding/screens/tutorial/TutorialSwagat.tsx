...
export default function TutorialSwagat({
  ...
  const LINES = isLatin ? [
    'Namaste Pandit Ji. HmarePanditJi mein aapka swagat hai.',
    'Yeh app poori tarah se aapki awaaz se chalega.',
    'Aapko koi button nahi dabana — bas mere sawaalon ke jawaab dijiye.',
    'Humara Mool Mantra yaad rakhiye — App Pandit ke liye hai, Pandit App ke liye nahi.',
  ] : [
    'नमस्ते पंडित जी। HmarePanditJi में आपका स्वागत है।',
    'यह app पूरी तरह से आपकी आवाज़ से चलेगा।',
    'आपको कोई button नहीं दबाना — बस मेरे सवालों के जवाब दीजिए।',
    'हमारा Mool Mantra याद रखिए — App पंडित के लिए है, पंडित App के लिए नहीं।',
  ]

  const TITLE = isLatin ? 'Namaste' : 'नमस्ते'
  const SUBTITLE = isLatin ? 'Pandit Ji.' : 'पंडित जी।'
  const WELCOME = isLatin ? 'Welcome to HmarePanditJi.' : 'HmarePanditJi में आपका स्वागत है।'
  const QUOTE = isLatin ? '"App Pandit ke liye hai, Pandit App ke liye nahi."' : '"App पंडित के लिए है, पंडित App के लिए नहीं।"'
  const CTA_LABEL = isLatin ? 'Samajh gaya — Shuru Karein 🪔' : 'समझ गया — शुरू करें 🪔'
  ...
  const { voiceFlowState } = useSarvamVoiceFlow({
    language: 'Hindi',
    script: currentLine < LINES.length ? LINES[currentLine] : '',
      ...
      <main className="relative z-10 flex-grow flex flex-col items-center px-6 xs:px-8 pt-6 pb-32 max-w-2xl mx-auto w-full">
        ...
        <motion.div className="w-full bg-white rounded-3xl p-6 xs:p-8 shadow-[0px_8px_24px_rgba(144,77,0,0.08)] mb-8 border-l-4 border-[#ff8c00]" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }}>
          <h1 className="font-headline text-2xl xs:text-3xl font-bold text-[#904d00] mb-3 leading-relaxed">
            {TITLE} <span className="text-[#ff8c00]">{SUBTITLE}</span>
          </h1>
          <p className="text-[#1b1c19] text-base xs:text-lg leading-[180%] font-medium mb-4">{WELCOME}</p>
          <div className="w-16 h-[2px] bg-[#e4e2dd] my-4" />
          <p className="text-[#564334] text-base xs:text-lg italic leading-relaxed">{QUOTE}</p>
        </motion.div>
