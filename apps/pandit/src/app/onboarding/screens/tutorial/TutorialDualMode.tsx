...
export default function TutorialDualMode({ onNext, onSkip, onBack }: TutorialDualModeProps) {
  ...
  const LINES = ['दो मोड — आपकी सुविधा।', 'पंडित मोड — जब आप पूजा करना चाहें।', 'ग्राहक मोड — जब आपको ग्राहक ढूंढना हो।', 'एक क्लिक में बदलें।', 'आगे बोलें।']
  ...
  return (
    <main className="w-full min-h-dvh max-w-[390px] xs:max-w-[430px] mx-auto bg-vedic-cream flex flex-col">
      ...
      <div className="flex-1 px-4 xs:px-6 py-6 xs:py-8 overflow-y-auto">
        ...
        {showCards && (<div className="space-y-3 xs:space-y-4 mt-6 xs:mt-8"><motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="bg-saffron-lt border-2 border-saffron rounded-xl p-4"><p className="font-bold text-text-primary">🙏 पंडित मोड</p><p className="text-sm text-text-secondary">पूजा करें, कमाई करें</p></motion.div><motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="bg-primary-lt border-2 border-primary rounded-xl p-4"><p className="font-bold text-text-primary">👤 ग्राहक मोड</p><p className="text-sm text-text-secondary">ग्राहक बनें, पूजा बुक करें</p></motion.div></div>)}
      </div>
