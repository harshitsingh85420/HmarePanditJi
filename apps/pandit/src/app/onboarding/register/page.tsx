    ...
    <div className="min-h-screen bg-[#f8f7f6]">
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            {/* Tutorial re-entry link */}
            <button
              onClick={() => {
                try {
                  const raw = localStorage.getItem("hpj_pandit_onboarding_v1");
                  const s = raw ? JSON.parse(raw) : {};
                  s.tutorialCompleted = false;
                  s.phase = "TUTORIAL_SWAGAT";
                  s.currentTutorialScreen = 1;
                  localStorage.setItem("hpj_pandit_onboarding_v1", JSON.stringify(s));
                } catch (_e) { /* ignore */ }
                router.push("/onboarding");
              }}
              className="flex items-center gap-1 text-[#9B7B52] text-xs font-medium px-2.5 py-1.5 rounded-full border border-[#F0E6D3] hover:bg-[#FEF9F0] active:scale-95 transition-all"
            >
              🎓 Tutorial देखें
            </button>
            <button
