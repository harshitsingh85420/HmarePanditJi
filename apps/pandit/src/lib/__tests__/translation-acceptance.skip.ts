 ...
 *
 * NOTE: This file contains usage examples and manual acceptance tests.
 * It requires a running API server and is skipped in automated test runs.
 * To run manually: execute the exported functions in browser console.
 */
...
// Skip this test suite - it's for manual usage examples only
describe.skip('Translation Acceptance Tests (Manual Only)', () => {
  it('skipped - requires running API server', () => {
    // This test is skipped because it requires:
    // 1. Running API server at localhost:3002
    // 2. Valid Sarvam API key
    // 3. Network connectivity to Sarvam AI services
    // Run exported functions manually in browser console instead
    expect(true).toBe(true);
  });
});

// Mock getScriptText function
async function getScriptText(_screenId: string, _language: string) {
  return 'mocked text';
}
...
async function testTranslationEngine() {
  ...
  const result = await translateEngine.translate({
    ...
    onResult: (text: string, _confidence: number) => {
      console.log('Translated:', text); // Expected: "स्वागत है पंडित जी"
      console.log('Confidence:', _confidence); // Expected: 0.95
    },
  });
