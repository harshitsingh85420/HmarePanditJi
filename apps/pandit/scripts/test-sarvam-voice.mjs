...
const API_KEY = process.env.SARVAM_API_KEY || process.env.NEXT_PUBLIC_SARVAM_API_KEY;
...
async function testTTS() {
  ...
  try {
    ...
    if (response.audios && response.audios.length > 0) {
      const audioBytes = Buffer.from(response.audios[0], 'base64').length;
      console.log(`✅ TTS SUCCESS — audio bytes received: ${audioBytes}`);
      console.log(`   Sample rate: 22050 Hz`);
      console.log(`   Model: bulbul:v3`);
      console.log(`   Speaker: priya`);
      return true;
    } else {
