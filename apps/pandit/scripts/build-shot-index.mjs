...
const GROUPS = {
  ...
  page18: { order: 18, title: "PAGE 18 · आपातकालीन SOS (the crown answer: SOS dials the pandit's own handset and does nothing else — no backend exists)" },
  page17: { order: 17, title: "PAGE 17 · मदद + FAQ (first page walked in WATCH mode — a real window on screen banked these; offline truths measured)" },
...
const CAPTIONS = {
  ...
  "page15/j1-settings-full.png": "सेटिंग्स in full: 15 controls, no back (tab screen), ONE toggle — and a mount line promising a voice switch that isn't here",
  ...
  "page18/m1-sos-rest.png": "SOS at rest: the 210px hold button (a tap does nothing — the gesture must be taught) + the support-team row",
  "page18/m2-hold-cancelled.png": "Fat-finger protection works: a 400ms press cancels — no dial, no state change",
  "page18/m3-sos-fired.png": "After a full 1200ms hold: the button turns GREEN with a success tick — while ZERO requests left the device (Finding 1)",
  "page18/m4-sos-offline.png": "Offline: renders, banner up, hold still works, claims nothing was sent in words — the honest behaviour",
  "page18/m5-pill-expanded.png": "The floating pill expanded: 'सहायता टीम को कॉल कीजिए' — 2 taps to a real call, faster than the screen it points to",
  "page17/l1-help.png": "मदद: 8 controls; the tel: and wa.me rows asserted present and NEVER fired; zero API calls by this screen",
  "page17/l2-faq-open.png": "THE FIX IN PIXELS: the booking-alert answer now reads phone-first AND app-open-qualified",
  "page17/l4-help-offline.png": "WARM OFFLINE: banner up, all 8 rows alive, tel: still dialable — the help screen survives a dead network",
  "page17/l5b-help-cold-offline.png": "COLD OFFLINE: the browser's own error page — no service worker, so nothing of the app renders",
