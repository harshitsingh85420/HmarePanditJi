...
interface BookingCTAProps {
    panditId: string;
    /**
     * TRACK 2A KILL-LIST 2+3/4. `null` means WE DO NOT KNOW HIS RATE — the
     * type carries the distinction so a caller cannot quietly pass 0 again.
     * The "Starting from" block is omitted entirely when null: an absent price
     * must not be dressed as a cheap one, and ₹0 advertised a free puja.
     */
                ...
                <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] p-4 z-50 flex items-center justify-between">
                    {lowestPrice === null ? (
                        <div>
                            ...
                            <p className="text-sm font-semibold text-gray-700">Not listed yet</p>
                        </div>
                    ) : (
                        <div>
                            <p className="text-xs text-gray-500">Starting from</p>
                            <p className="text-xl font-bold text-gray-900">₹{lowestPrice.toLocaleString("en-IN")}</p>
                        </div>
                    )}
                    <button
                ...
                <div className="flex items-center gap-6">
                    {lowestPrice === null ? (
                        <div className="text-right">
                            <p className="text-sm text-gray-500">Dakshina</p>
                            <p className="text-base font-semibold text-gray-700">Not listed yet</p>
                        </div>
                    ) : (
                        <div className="text-right">
                            <p className="text-sm text-gray-500">Starting from</p>
                            <p className="text-2xl font-bold text-gray-900">₹{lowestPrice.toLocaleString("en-IN")}</p>
                        </div>
                    )}
                    <button
                        onClick={handleBookClick}
