...
| PW-07 | **FOUNDER REGISTER LAW (Isj, 2026-07-21): शुद्ध, सम्मानजनक हिंदी in everything the app writes OR speaks.** आप-forms only; imperatives in the -इए form (कीजिए/बोलिए/दबाइए/चुनिए/भेजिए); छूकर/छुओ/टच → स्पर्श कीजिए; तुम/तू nowhere ever; पंडित जी always with जी. Bar: "how a devoted शिष्य addresses his guru" — simple, warm, आदरपूर्ण, never over-Sanskritized. Hierarchy: Isj's language directives > canon copy > legacy strings (truthful-state trumps all). **AUTO-guarded** by `apps/pandit/src/lib/registerLaw.test.ts` (banned-token scan with Devanagari-aware boundaries over strings.ts + voice-scripts + component source; voice-GRAMMAR arrays exempt — the app must keep understanding casual speech it never uses itself; proof-of-teeth fixtures included). Swept 2026-07-21 (~300 strings, table in docs/register-sweep-2026-07-21*.txt), pending Isj's line-by-line ratification. | The doc specced Devanagari + voice-first but never the *register* of the Hindi itself. |
| PW-08 | **Quoted vocabulary is exempt from PW-07, verbatim-preserved:** native-browser button names ('अनुमति दें'/'Allow' — the OS renders them, the app must name them exactly), and taught user-commands ('वापस करो', 'हटा दो', 'सो जाओ' — the user's own words, which the app quotes while teaching). The guard's allowlist is the single source of these exemptions. | Follows from PW-01 (OS owns its popups) + F02's command grammar. |
