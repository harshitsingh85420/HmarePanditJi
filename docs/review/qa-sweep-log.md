...
> **Next page (protocol): PAGE 13 · मेरी पूजाएँ** (my-poojas list + add-wizard entry); wizard R2-R5 queued as pages after the dashboard arc. §3-V visibilityAudit is now PART OF EVERY PAGE's §3 (scripts/lib/visibilityAudit.mjs).
...
**NOTES (report):** TZ keying asymmetry — booking keys use LOCAL date parts, blocked keys use getUTC* (server stores UTC-midnight); consistent today, but a non-IST device near midnight could show a dot and its ✕ on different days — unify someday. **"F19-1" does not exist in the repo** — the per-day date-lock ALREADY ships end-to-end (client toggle + API 409 has_booking guard); if the founder's parked-feature list says otherwise, it's stale.
